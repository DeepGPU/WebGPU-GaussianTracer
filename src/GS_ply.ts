import { PackingType, StaticArray, Struct, vec3, vec4, f32 } from "./GS_packing";
import { Vec3 } from "wgpu-matrix";

export async function loadGaussianSplatting(filename: string): Promise<PackedGaussians> {
    return fetch(filename)
        .then(response => response.arrayBuffer())
        .then(arrayBuffer => new PackedGaussians(arrayBuffer));
}
